...
class graphcl(nn.Module):
    def __init__(self, gnn):
        ...
        #augmenataion 방법이 5개니까 projection head 5개 만듬
        self.projection_head = nn.ModuleList([nn.Sequential(nn.Linear(300, 300), nn.ReLU(inplace=True), nn.Linear(300, 300)) for _ in range(5)])
...
def train(args, loader, model, optimizer, device, gamma_joao):
    ...
    n_aug1, n_aug2 = n_aug//5, n_aug%5    #matrix라고 생각하면 n_aug의 위치가 n_aug1 x n_aug2 위치
    ...
    # for step, batch in enumerate(loader, desc="Iteration"):
        ...
        x1 = model.forward_cl(batch1.x, batch1.edge_index, batch1.edge_attr, batch1.batch, n_aug1)
        x2 = model.forward_cl(batch2.x, batch2.edge_index, batch2.edge_attr, batch2.batch, n_aug2)
    ...
    for n in range(25):
        ...
        count, count_stop = 0, len(loader.dataset)//(loader.batch_size)+1
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if epoch % 20 == 0:
            if 'abide' in args.root_unsupervised:
                ...
                torch.save(model.gnn.state_dict(), "./weights/abide/joaov2_" + str(args.gamma) + '_' + str(args.gnn_type) + '_' + 'batch' + str(args.batch_size) + '_' + unsupervised_path + '_' + str(epoch) + ".pth")
            else:
                unsupervised_path = str(args.root_unsupervised).strip("/abide/")
                torch.save(model.gnn.state_dict(), "./weights/joaov2_" + str(args.gamma) + '_' + str(epoch) + '_' + str(args.gnn_type) + '_' + 'batch' + str(args.batch_size) + '_' + unsupervised_path + '_' + str(epoch) + ".pth")
