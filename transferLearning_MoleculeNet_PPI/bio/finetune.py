...
def main():
    ...
    model_graphcl.load_state_dict(checkpoint['graphcl'])
    ...
    with open('./results/'+args.resultFile_name+'.res', 'a+') as f:
        f.write(str(args.runseed) + ' ' + str(np.array(val_acc_list).max()) + ' ' + str(np.array(test_acc_hard_list)[np.array(val_acc_list).argmax()]))
        f.write('\n')
